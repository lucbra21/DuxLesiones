...
def view_registro_lesion(modo: str = "nuevo", jugadora_info: str = None, lesion_data = None) -> None:
    ...
    if lesion_data and lesion_data["estado_lesion"] == "INACTIVO":
        ...
        if fecha_observacion_inactiva:
            st.warning(f"{t('La lesión se encuentra en está en estado **Inactiva** desde el')} {fecha_observacion_inactiva}.")
        else:
            st.warning(f"{t('La lesión esta **Inactiva**, **fecha de alta médica:**')} {fecha_alta_medica}, {t('**fecha de alta deportiva:**')} {fecha_alta_deportiva}. {t('No se pueden editar los datos')}")
    ...
    with col1:
        fecha_lesion = st.date_input(t("Fecha de la lesión *"), fecha_lesion_date, 
        ...
        segmento = st.selectbox(t("Región anatómica *"), segmentos_corporales_list, 
                                index=idx_segmento, disabled=disabled_edit, placeholder=placeholder, 
                                key=f"segmento_{st.session_state['form_version']}")
    with col2:
        lugar = st.selectbox(t("Lugar *"), lugares_list, index=idx_lugar, disabled=disabled_edit, placeholder=placeholder, 
        ...
        zona_cuerpo = st.selectbox(t("Zona anatómica *"), opciones_tipo_zona, index=idx_zonas, disabled=is_disabled, 
                                   placeholder=placeholder, key=f"zona_cuerpo_{st.session_state['form_version']}")
    with col3:
        mecanismo_lesion = st.selectbox(t("Mecanismo de lesión *"), mecanismo_list, index=idx_mecanismo, disabled=disabled_edit, 
        ...
        zona_especifica = st.selectbox(t("Estructura anatómica *"), opciones_tipo_zona_especifica, index=idx_zona_espec, 
    ...
    with col4:
        ...
        tipo_lesion = st.selectbox(t("Tipo de lesión *"), opciones_tipo_lesion, index=idx_tipos_lesion, 
                                   disabled=is_disabled, help=lesion_help, placeholder=placeholder, 
                                   key=f"tipo_lesion_{st.session_state['form_version']}")
        
        lateralidad = st.selectbox(t("Lateralidad"), lateralidades, index=idx_lateralidad, disabled=disabled_edit, placeholder=placeholder, key=f"lateralidad_{st.session_state['form_version']}")
    with col5:
        ...
        tipo_especifico = st.selectbox(t("Tipo específico *"), opciones_tipo, index=idx_tipo_especifico, disabled=is_disabled, help=lesion_help, placeholder=placeholder, key=f"tipo_especifico_{st.session_state['form_version']}")

        ############################################

    diagnostico = st.text_area(t("Diagnóstico Médico"), disabled=disabled_edit, value=diagnostico_text, key=f"diagnostico_{st.session_state['form_version']}")

    col1, col2, col3, col4 = st.columns([1,2.5,1,2.5])    

    with col1:
        es_recidiva = st.checkbox(t("Es Recidiva"), value=es_recidiva_value, disabled=disabled_edit, 
    ...
    with col2:
        tipo_recidiva = st.selectbox(
                ...
                help=t("Clasificación basada en la fisiología de la reparación tisular y en la evidencia epidemiológica de la UEFA y el IOC."),
    ...
    with col3:
        ...
        implica_baja = st.checkbox(t("Implica Baja"), value=implica_baja_value, disabled=disabled_edit, 
    ...
    with col4:
        if implica_baja:
            ...
            fecha_alta_diagnostico = st.date_input(t("Alta Deportiva (estimada)"), value=datetime.date.today() + datetime.timedelta(days=1),
            ...
            fecha_alta_diagnostico = st.date_input(t("Alta Deportiva (estimada)"), value=None,
                                                disabled=True if not implica_baja or disabled_edit else False, 
    ...
    if fecha_observacion_activa_date:
        st.info(f"{t(':material/calendar_clock: La lesión se encuentra en estado:')} **{estado_lesion}** {t('desde el dia')} {fecha_observacion_activa_date.strftime('%Y-%m-%d')}")
    ...
    with col1:
        tipo_tratamiento = st.multiselect(t("Tipo(s) de tratamiento"), options=tratamientos_list, default=tratamientos_default, 
    ...
    with col2:
        personal_reporta = st.text_input(t("Personal médico que reporta *"), value=personal_reporte_text, disabled=disabled_edit, 
                                         key=f"personal_reporta_{st.session_state['form_version']}")

    descripcion = st.text_area(t("Observaciones / Descripción de la lesión"), value=descripcion_text, disabled=disabled_edit, 
    ...
    if modo == "editar":
        ...
        seguimiento = st.checkbox(t("Añadir seguimiento"), disabled=disabled_evolution, 
        ...
        with col1:
            fecha_control = st.date_input(t("Fecha de control"), datetime.date.today(), max_value=datetime.date.today(), disabled=disabled_evolution,
                                          key=f"fecha_control_{st.session_state['form_version']}")
        with col2:
            tratamiento_aplicado = st.multiselect(t("Tratamiento Aplicado"), tratamientos_list, placeholder="Selecciona uno o más", 
                                                  max_selections=15, disabled=disabled_evolution,
                                                  key=f"tratamiento_aplicado_{st.session_state['form_version']}")
        with col3:
            personal_seguimiento = st.text_input(t("Personal médico *"), disabled=disabled_evolution,
                                                 key=f"personal_seguimiento_{st.session_state['form_version']}")

        incidencias = st.text_area(t("Observaciones o incidencias"), disabled=disabled_evolution,
        ...
        if not fecha_observacion_activa_date and not fecha_observacion_inactiva_date and not fecha_alta_diagnostico_date:
            ...
            with col1:
                alta_medica = st.checkbox(t("Alta Médica"), value=alta_medica_value, disabled=alta_medica_value or disabled_evolution)
            ...
            with col2: 
                if alta_medica_value:
                    alta_deportiva = st.checkbox(t("Alta Deportiva"), value=alta_deportiva_value, disabled=disabled_evolution)
